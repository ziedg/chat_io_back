               ...
               res.send(err.status); 
				...
				Profile.findById(body.profileId, function(err, profile) {
                    ...
                    res.json({
						...
						profile.profilePicture = "http://91.121.69.130/images/"+req.files.profilePicture[0].filename;
						profile.profilePictureMin = "http://91.121.69.130/images/"+ req.files.profilePicture[0].filename;
						}
						profile.save();
						res.json({
							status: 0,
		...
		if (err){
			...
			require("fs").writeFile("/var/www/html/images/"+fileName, base64Data, 'base64', function(err) {
				if(err){
					...
					profile.profilePicture="http://91.121.69.130/images/"+fileName;
					profile.profilePictureMin="http://91.121.69.130/images/"+fileName;	
					profile.save();
				}
			});
		}	
